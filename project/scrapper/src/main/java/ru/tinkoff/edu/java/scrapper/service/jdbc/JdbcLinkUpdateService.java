...
public class JdbcLinkUpdateService implements LinkUpdateService {
    ...
    public void updateLinks() {
        ...
        for (Link link : oldLinks) {
            Link_Parser result = linkParser.parseUrl(link.getUrl());
            if (result instanceof Github) {
                try {
                    System.out.println(link.getUrl());
                    GitHubResponse response = gitHubClient.fetchRepo(((Github) result).username(), ((Github) result).repository());
            ...
            } else if (result instanceof StackOverflow) {
                try {
                    System.out.println(link.getUrl());
                    StackOverflowItem response = stackOverflowClient.fetchQuestion(((StackOverflow) result).id());
